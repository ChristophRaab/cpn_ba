...
#            print(next_shape)
    ...
    def _loss(self, X, Y_one_hot):
        ...
        for layer in self.layers:
            X_next = layer.fprop(X_next)
        Y_pred = X_next
        return self.layers[-1].loss(Y_one_hot[:300], Y_pred)
    ...
    def predict(self, X, batch_size=256):
        ...
        while offset < count:
            old_offset = offset
            offset = min(old_offset + batch_size, count)
            X_next = X[old_offset:offset]

            for layer in self.layers:
                X_next = layer.fprop(X_next)

            Y_pred = np.concatenate((Y_pred, unhot(X_next)), axis=0)
